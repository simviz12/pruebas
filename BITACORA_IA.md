...
### Fase 7 y 8 (DocumentaciÛn y Cierre):
- **Rechazo al uso de LLMs (Prompt 17):** Se considerÛ inicialmente usar un LLM (como ChatGPT/Claude) de forma din·mica en producciÛn para extraer la informaciÛn de los correos. **Se rechazÛ rotundamente**. Un LLM en este flujo transaccional aÒade latencia inaceptable, costos por token innecesarios, y no es determinista (puede alucinar datos bancarios). Se optÛ por Expresiones Regulares (Regex) deterministas, r·pidas y exactas.
- **Commits Progresivos (Prompt 20):** Se mantuvo un historial atÛmico usando *Conventional Commits* en lugar de un solo commit masivo.
